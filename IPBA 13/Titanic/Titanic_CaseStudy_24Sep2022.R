...
data <- data %>% mutate(Transported_Flag = if_else(Transported == "True",1,0))
...
# Does is have Outliers?
g <- ggplot(data, aes(Age)) + geom_boxplot() + coord_flip()
g
...
# Treat HomePlanet
data$HomePlanet <- ifelse(is.na(data$HomePlanet)
                          , "missing", data$HomePlanet)

# Times transported HomePlanet
g <- ggplot(data, aes(HomePlanet)) + geom_bar()
g

# % transported 
# Step 1 Summarize Data
HomePlanet_Summary <- data %>%
  group_by(HomePlanet)  %>%
  summarise(
    n_passengers = n(),
    sum_Transported_Flag = sum(Transported_Flag)
  )

# Step 2 Create/Mutate to create % Column
HomePlanet_Summary <- HomePlanet_Summary %>% 
  mutate(Percent_Transported = sum_Transported_Flag/n_passengers)


# Step 3 Plot
g <- ggplot(HomePlanet_Summary, aes(x = HomePlanet,y = Percent_Transported)) + geom_col()
g
...
# Treat Destination
data$Destination <- ifelse(is.na(data$Destination)
                          , "missing", data$Destination)

# No of Destination
g <- ggplot(data, aes(Destination)) + geom_bar()
g
